...
## 本项目特有规则：监控工作流必须看 Pages 页面（关键）

- **每次我让监控工作流是否符合预期，必须同时检查两个维度**：
  1. 工作流运行日志（是否报错、数据量、耗时）
  2. **Pages 页面内容**（数据日期、总车型数、双源核验数、筛选功能等）
- **如果 Pages 页面不符合预期，必须反推原因**：
  - 数据没更新 → 是合并分析没跑？还是爬虫没产出有效数据？
  - 数据量不对 → 是合并逻辑问题？还是爬虫数据不完整？
  - 双源核验数不对 → 是合并逻辑问题？还是只有一个数据源？
- **禁止**：只看工作流日志不看 Pages 页面就下结论
- **已有案例**：看了日志说懂车帝超时，没看 Pages 页面确认数据日期，实际页面还是 6月15日旧数据
...
- **N 选 M 通过制 + 懒启动**：先并行 N 个（如 N=2），全部通过不启动更多；不达标再逐启动 N+1/N+2
...
2. 从剩余 Review category 中选 2 个并行评审
...
> - 无需再次询问用户，直接并行启动评审
>
> ⚠️ **服务器资源限制**：当前生产服务器为 2核4G，2 个 Oracle 并行评审峰值额外内存消耗约 60-160MB。
