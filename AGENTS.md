...
- **修改任何文件前，必须理解该文件的数据流和命名规则。禁止凭猜测写代码。**
- **必须阅读相关代码确认以下信息后再修改**：
  - 变量/函数的命名规则（如文件名是 ID 还是字母？字段名有无映射？）
  - 数据的生产和消费链路（谁生成、谁读取、什么格式？）
  - 关键常量和配置的实际值（阈值是多少？开关在哪里？）
- **禁止**：不读代码直接写修复，导致逻辑与实际情况完全不符
- **禁止**：不确认文件命名规则就写 glob/正则匹配
- **已有案例**：HTML 文件命名是数字 car_id（如 `12345`），写了 `f"{letter}*"` 按字母匹配，永远匹配不到

## 禁止绕过评审直接创建标记（关键）

- **`mark-review-passed` 前必须先并行调用 3 个 Review Agent 完成评审，无例外。**
- **禁止**：修改代码后直接 `mark-review-passed` 跳过评审
- **禁止**：改完代码立刻 commit，中间没有 Review Agent 的评审结果
- **已有案例**：两次修改后直接 `mark-review-passed` 未调用 Review Agent，被用户发现

## 禁止治标不治本（关键）

- **找到问题后必须先定位根因再修复，禁止只看现象不看本质。**
- **必须沿着数据链路逐层排查**：输入 → 中间状态 → 输出，找到最早断裂点
- **禁止**：看到一个中间状态异常就去修补那个状态，而不追溯到源头
- **已有案例**：看到"双源核验只有 307"就去改合并逻辑，实际根因是汽车之家 artifact 只有 162 bytes；看到"增量超时"就只改时间限制，实际根因是 INCREMENTAL_MODE=false 且每次删 HTML
