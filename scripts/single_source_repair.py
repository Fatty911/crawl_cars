...
置信度评估要求：overall "confidence" 表示你对 approved_candidate_ids 整体批准集合的信心。当每个批准的候选都有明确证据（列表内候选、样本一致、无冲突）时，confidence 必须 >= 0.9；只有当部分批准证据不足或存在明显冲突时，才允许 0.5..0.85。不要因为个别小疑问就给出低分——证据充分的批准应得到高分，否则每轮都会被 0.85 门槛拦截而无法推进。
{{
  "should_fix": true,
  "confidence": 0.0,
