...
### Fixed
- 易车数据在进入 Pages 发布集前按车辆级别剔除货车、卡车、皮卡、轻客、微面等商用车；不再按品牌整体排除，避免误删同品牌的 MPV、SUV 和微型乘用车。

## 2026-07-19

### Fixed
- 懂车帝爬虫列名归一化：`_v4` 后缀英文字段（laser_radar_v4、driving_assist_chip_v4 等）映射为中文名；选装包/套装/订阅类字段（`包名_数字` 格式）归一化为去掉后缀的列名，并新增 `选装包列表` 汇总列。
- merge_data.py 扩展 HEADER_MAP 同义词映射表，覆盖三源（汽车之家/懂车帝/易车）的字段名差异，统一格式后缀（`(个)` vs `_个_` vs `[个]`）、同义词（`后悬挂形式` vs `后悬架类型`）、英文转中文（`departure_angle` → `离去角`、`filter_group_*` → `*分组` 等）。
- merge_data.py `norm()` 函数新增 `_数字` 后缀归一化（选装包字段）和 `_v4_值` 归一化（one-hot 编码的智驾/座舱配置值）。
- 汽车之家爬虫 workflow 新增下载最新 merged release 数据步骤，使 `prioritize_series_queue` 能基于最新数据优先爬取懂车帝已有但汽车之家缺失的车系，提升双源重叠率。

### Changed
- 字段总数从 5,409 个减少至约 200–300 个核心字段，显著提升 Pages 筛选中心的可用性。
...
- Pages 筛选中心默认按车系聚合卡片，显示符合条件的车型数并支持点击展开具体型号；价格升序/降序分别以各车系最低价/最高价车型作为代表比较。
...
- 增加车系聚合、价格极值代表车型、无价车系排尾、缺失车系键、展开明细和按车系加载上限测试。
