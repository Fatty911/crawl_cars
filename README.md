...
| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| `RUN_TIME` | 每轮运行最大秒数 | 21600 (6小时) |
| `MAX_CARS` | 每轮最多爬取车型/车系数 | 400 |
...
| Job名 | 功能 | 超时 | 依赖 |
|-------|------|------|------|
| `crawl-autohome` | 爬取汽车之家 | 360分钟 | 无 |
| `crawl-dongchedi` | 爬取懂车帝 | 360分钟 | 无 |
...
**自动运行逻辑**：
1. 开始前随机延迟5-15分钟；外部随机触发会补足到至少30分钟
2. 爬取循环：
   - 运行指定时间或达到数量上限
...
**当前配置**（按最长运行时长估算，实际会因进度和数量上限提前结束）：
