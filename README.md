...
3. 每个主爬虫 workflow 有并发锁，同一爬虫同一分支只允许一个运行；备用触发会排队或跳过，避免并发重复爬
4. 同一个半月周期内如果已完成全量爬取，后续自动触发会直接跳过；进入新半月周期时自动重置对应爬虫进度
5. 如果 GitHub Actions schedule 被延迟到北京时间 12:00 以后或 13:30 以后，工作流会直接跳过，不在傍晚补跑
6. 懂车帝重置进度时会保留车系列表缓存，接口短暂返回非 JSON 或空响应时可回退继续爬取
7. 每两次网络访问之间默认等待3-8秒，模拟人工浏览动作速率
