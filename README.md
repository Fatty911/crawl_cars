...
**触发条件**：
- 汽车之家、懂车帝：每天 UTC 01:07（北京时间 09:07 左右）运行上午窗口，约 3 小时
- 汽车之家、懂车帝：每天 UTC 05:07（北京时间 13:07 左右）运行下午窗口，约 5 小时 50 分钟
- 合并分析：每天 UTC 12:30（北京时间 20:30），等待下午爬虫窗口结束后再合并
- 随机触发器只在北京时间 09:00-12:00 或 13:00-13:30 触发爬虫
- 手动触发 (workflow_dispatch)

**自动运行逻辑**：
1. 开始前随机延迟0-10分钟；外部随机触发最多补足到30分钟，并会封顶在当前运行窗口结束前
2. 爬取循环：
   - 按当前运行窗口运行指定时长
...
4. 如果 GitHub Actions schedule 被延迟到北京时间 12:00 以后或 13:30 以后，工作流会直接跳过，不在傍晚补跑
5. 懂车帝重置进度时会保留车系列表缓存，接口短暂返回非 JSON 或空响应时可回退继续爬取
6. 每两次网络访问之间默认等待3-8秒，模拟人工浏览动作速率
...
| 爬虫 | 定时 | 次数/月 | 分钟数 |
|------|------|---------|--------|
| 汽车之家 | 每天 UTC 01:07 与 05:07（北京时间 09:07、13:07 左右） | 最多 60 | 半月全量完成后跳过 |
| 懂车帝 | 每天 UTC 01:07 与 05:07（北京时间 09:07、13:07 左右） | 最多 60 | 半月全量完成后跳过 |
| 合并 | 每天 UTC 12:30（北京时间 20:30） | 30 | <300 |

**半月跳过**：每个爬虫在当月 1-15 日、16-月底两个周期内全量完成后，会写入 `crawl_state/` 完成标记；同一周期后续自动触发直接跳过，不再重复爬。

**随机延迟**：开始前随机等待 0-10 分钟；两次网络访问之间默认等待 3-8 秒，可通过 `CRAWL_MIN_DELAY_SECONDS` / `CRAWL_MAX_DELAY_SECONDS` 调整。
...
**当前调度**（UTC时间）：
- 汽车之家：每天 01:07（北京时间 09:07 左右，约 3 小时）和 05:07（北京时间 13:07 左右，约 5 小时 50 分钟）
- 懂车帝：每天 01:07（北京时间 09:07 左右，约 3 小时）和 05:07（北京时间 13:07 左右，约 5 小时 50 分钟）
